data{
	...
	vector[p] v; //extract temperature variable
}
parameters{
	...
	real betaV; //slope between slope and temperature
...
model{
	for(i in 1:p){ //loop over locations
		beta1[i] ~ normal(betaV*v[i],beta1_sd); //location-specific slope is a function of temperature at the location
